          ...
          <div className="grid">
            <div className="panel">
              ...
              {preview ? (
                <div className="imgWrap">
                  <img src={preview} className="img" alt="preview" />
                </div>
              ) : (
                <div className="emptyBox">No image selected</div>
              )}

              <button className="btnPrimary" onClick={handleConvert} disabled={loading}>
            ...
            <div className="panel">
              ...
              {result ? (
                ...
                <div className="emptyBox">Result will appear here</div>
              )}
            </div>
          </div>
        </div>
...
const css = `
  ...
  *{ box-sizing:border-box; }
  ...
  /* 🌌 Background (REALISTIC like BgRemove) */
  ...
  .heroSub{ margin:10px 0 0; opacity:.78; font-size: 13px; }
  ...
  .uploadGlow{
    position:absolute;
    inset:-2px;
    background: radial-gradient(600px 180px at 20% 0%, rgba(34,211,238,.22), transparent 55%),
                radial-gradient(700px 220px at 80% 100%, rgba(124,92,255,.18), transparent 55%);
    opacity:.9;
    pointer-events:none;
    filter: blur(10px);
  }
  .uploadMain{ position:relative; font-weight: 1000; }
  .uploadHint{ position:relative; font-size: 12px; opacity:.75; }
  ...
  .btnDownload{
    color: #0b1020;
    background: linear-gradient(135deg, rgba(52,211,153,.92), rgba(255,190,60,.88));
    box-shadow: 0 18px 40px rgba(0,0,0,.20);
  }
  .btnPrimary:hover, .btnDownload:hover{ transform: translateY(-1px); filter: brightness(1.04); }
